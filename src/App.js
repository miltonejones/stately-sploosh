...
import { getVideos, getModels, getModelsByName, deleteVideo, getVideosByDomain, updateModelPhoto, toggleVideoFavorite, findVideos, getFavorites, getVideoKeys} from './connector';
...
function Application() {
  ...
  const finder = useSearchDrawer(val => !!val && navigate(`/search/1/${val}`));
  const saving = ['save.next','save.load','search.next','search.page','search.find'].some(shop.state.matches);
...
<Avatar src="https://s3.amazonaws.com/sploosh.me.uk/assets/sploosh.png" alt="sploosh"/>


  <Btn size="small" variant={state.matches('dash') ? 'contained' : 'text'} onClick={() => navigate('/dash')}>home</Btn>
  <Btn size="small" variant={!state.matches('dash') && ['video','domain','search'].some(f => view  === f)  ? 'contained' : 'text'} onClick={() => navigate('/video/1')}>videos</Btn>
  <Btn size="small" variant={view === 'favorites' ? 'contained' : 'text'} onClick={() => navigate('/favorite/1')}>favorites</Btn>
  <Btn size="small" variant={view === 'recent' ? 'contained' : 'text'} onClick={() => navigate('/recent/1')}>recently watched</Btn>
  <Btn size="small" variant={view === 'model' ? 'contained' : 'text'} onClick={() => navigate('/model/1')}>models</Btn>
 ...
 <Box sx={{flexGrow: 1}} />
  ...
  <IconTextField  
    endIcon={!search_param ? null : <i onClick={() => navigate('/video/1')} className="fa-solid fa-xmark" /> }
    placeholder="Type a title or model name"
    label="Search"
    onKeyUp={e => e.keyCode === 13 && navigate(`/${view === 'model' ? 'model' : 'search'}/1/${search_param}`)}
    onChange={e => {
      send({
        type: 'CHANGE',
...
<i onClick={() =>  shop.handleClick()} className={busy || saving ? "fa-solid fa-rotate App-logo" : "fa-solid fa-cart-shopping"}></i>
...
[{editor.videos?.length}] */}
  ...
  {!!tabList && !state.matches('dash') && view !== 'model' && <Flex 
  
    sx={{ borderBottom: 1, borderColor: busy ? 'primary' : 'divider' }}>


    {!!param && <Flex sx={{ml:  2}} spacing={2} >
    ...
    <i onClick={() => {
      const parameter = param.indexOf('*') > 0
        ? param.replace('*', '')
        : `${param}*`;
      navigate(`/search/1/${parameter}`)
    }} className={`${param.indexOf('*') > 0 ? 'red fa-solid' : 'fa-regular'} fa-heart`}></i>
    </Flex>}
  ...
  variant="scrollable"
  scrollButtons
  value={tabList.map(f => f.param).indexOf(param?.replace('*','')) + 1} 

  
  >
    <TabButton label="All videos" onClick={() => navigate('/video/1')} sx={{textTransform: 'none' }} />
    {tabList.map (tab => <TabButton sx={{textTransform: 'none', height: 24}} 
      icon={<i onClick={() => removeTab(tab.param)} className="fa-solid fa-xmark"></i>}
      iconPosition="end"
      key={tab.param} label={<Tooltip title={tab.param}><Typography 
      variant="body2"
      onClick={() =>{
        navigate(`/${tab.type}/1/${tab.param}`)
      }}>
     {tab.param.substr(0,15)}{tab.param.length > 15 && <>...</>}
      </Typography></Tooltip>} />)}
   </TabList>
    </Flex>}
  ...
  {(busy || saving) && <LinearProgress />}
    ...
    {view === 'model' && !state.matches('dash') && !!state.context.models && (
      ...
      <Flex spacing={1} sx={{ mt:1 }}>
        ...
        {!!param && <Typography onClick={() => navigate('/model/1')} variant="caption">
          {" "}like "<b>{param}</b>" <i className="fa-solid fa-xmark" />
        </Typography>}
        </Flex>
    <ModelGrid modelClicked={modal.openModel} models={state.context.models.records} />
      </Stack>
    )}


    {!state.matches('dash') && !!state.context.videos && view !== 'model' && <Stack spacing={1}>
  ...
  <PhotoGrid sx={{width: '97vw'}}>
      {state.context.videos.records.map(video => <VideoCard  medium
