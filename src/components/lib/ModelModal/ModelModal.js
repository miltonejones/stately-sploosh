...
import { styled, Avatar, Chip,  LinearProgress, Box, TextField, Pagination, Dialog, Stack, Typography } from '@mui/material';
...
export const useModelModal = () => {
  ...
  const [state, send] = useMachine(modelMachine, {
    services: {
      ...
      castModel: async(context) => {
        ...
        return await addModelToVideo(context.selected[context.add_index], context.ID); 
      },
      loadMissing: async (context) => { 
        return await getModelMissingVideos(context.ID) 
      }, 
      loadModel: async (context) => { 
        return await getModel(context.ID, context.page, context.favorite, context.filterText) 
      }, 
  ...
  const handleRename = (alias) => send({
    type: 'RENAME',
    alias
  })
  const handleChange = (value) => send({
    type: 'CHANGE',
    value
  })
...
const ModelModal = ({ open, agent, state, tab: tabnum, memory, missing, selected, progress, handleAlias, filterText, photoClicked,
  favoriteClicked, deleteClicked, searchClicked, handleSelect, handleBatch, aliasMode, handleRename, refresh,
  setFavorite, handleChange, favorite, setTab, costars, openModel, handleClose, setPage, page = 1, model }) => {
  ...
  const recordCount = Math.abs(tab) === 1 ? costars?.length : model.videos.count; 
   ...
   <Dialog maxWidth="md" open={open} onClose={handleClose}>
    ...
    <Stack spacing={1} sx={{p: 2, minHeight: 500, position: 'relative'}}> 
      <Stack direction="row" spacing={1} sx={{pr: 2}}>
        ...
        ><Avatar sx={{width: 48, height: 48}} variant="rounded" src={star.image} /></ModelMenu>
        <Stack>


          <Typography sx={{lineHeight:1}} variant="h6">{star.name}</Typography>
  {/* [{agent.status}] */}

      {!!model.aliases?.length &&  <Typography sx={{lineHeight:1}} variant="caption">AKA {model.aliases.map(alias => (
        <>
      <U onClick={() => openModel(alias.ID)}> {alias.alias}</U>, 
        ...
        {/* <Typography variant="caption"><u onClick={handleAlias}><b>Add alias</b></u></Typography> */}
          ...
          <I onClick={setFavorite} className={`fa-${favorite ? "solid" : "regular"} fa-star`}></I>

          <Box sx={{ flexGrow: 1}} />

          {!!aliasMode && <ModelSelect onValueSelected={val => !!val && handleRename(val)}/>}
        {!aliasMode && <IconTextField 
          endIcon={!filterText ? null : <i onClick={() => handleChange('')} className="fa-solid fa-xmark" /> }
    ...
    <Flex spacing={2}  >
      {!!missing?.length && <Chip variant={tab  === 2 ? 'contained' : 'outlined'} color="primary" label="Missing" color="error" size="small" onClick={() => setTab(2)}/>}


<i onClick={() => setTab(tab < 0 ? 0 : -1)} className="fa-solid fa-film"></i>
...
<i onClick={handleAlias} className="fa-solid fa-people-arrows"></i>
      ...
      {!!memory?.length && <ModelMenu selectedID={star.ID} onChange={e => !!e && openModel(e)} 
            models={memory}><I className="fa-solid fa-ellipsis-vertical"></I></ModelMenu>}
      {/* [{memory?.length}] */}
    </Flex>
    ...
    {pageCount > 1 && tab <  2 && (
      <Stack sx={{ ml:1, alignItems: 'center'}} direction="row" spacing={1}>
              <Chip variant={tab  === 0 ? 'contained' : 'outlined'} color="primary" label="Videos" size="small" onClick={() => setTab(0)}/>
      <Chip variant={Math.abs(tab)  === 1 ? 'contained' : 'outlined'} color="primary" label="Costars" size="small" onClick={() => setTab(1)} />
        <Pagination
          count={pageCount}
          page={page}
          onChange={setPage}
    ...
    <Grid wide>
      {visible?.map(star => <ModelCard modelClicked={openModel}  small model={star} />)}
    </Grid>
    {/* <pre>
...
{JSON.stringify(selected)} */}
      {tab === 0 && <Grid>
     {model.videos.records.map(record => <VideoCard 
    ...
    {!!progress && <LinearProgress variant="determinate" value={progress} />}{progress}
    </Box>
    <Stack direction="row">
    <u onClick={()  => handleSelect(missing.map(f  => f.ID))}>select all</u> 
     <Chip label={`Add model to  ${selected?.length} videos` } color="warning" onClick={handleBatch} />
    </Stack>
      <Grid>
     {missing.map(record => <VideoCard  
