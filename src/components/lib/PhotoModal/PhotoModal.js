...
import {  Snackbar, Dialog, LinearProgress, 
  Card,
  Typography,
  CardMedia,
  CardContent, Stack } from '@mui/material';
...
const PhotoModal = ({ photo, choosePhoto, state, name, open }) => {
  const size = 200;
 return (
   <Snackbar maxWidth="md" open={open} onClose={() => choosePhoto()} data-testid="test-for-PhotoModal">
    <Card>
...
{/* {JSON.stringify(state.value)} */}
     ...
     {!!photo && state.matches('opened.loaded') && <Stack direction="row">
        {photo?.map(pic =>  <PreviewCard src={pic.src} size={size} caption={pic.alt} onClick={() => choosePhoto(pic.src)} />)}
