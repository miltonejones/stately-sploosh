...
import { LinearProgress, styled, Drawer, Box, Stack, Typography } from '@mui/material';
...
export const useSearchDrawer = (onSearch) => {
  ...
  const handleDrop = (value) => send({
    type: 'DROP',
    value
  })
  const handlePin = (value) => send({
    type: 'PIN',
    value
  })
...
const SearchDrawer = ({ handleClose, handlePin, handleDrop, searches, state, open }) => {
  const pinned = searches?.filter(text => !!text && text.indexOf('^') > 0);
  const unpinned = searches?.filter(text => !!text && text.indexOf('^') < 0);
 return (
  <Drawer anchor="left" onClose={e => handleClose()} open={open} data-testid="test-for-VideoDrawer">
   ...
   {pinned?.map(item => <Line direction="row" sx={{ alignItems: 'center', width: 400}}>
      <Text onClick={() => handleClose(item.replace('^', ''))}>{item.replace('^', '')}</Text>
      <Box sx={{ flexGrow: 1}} />
      <i onClick={() => handlePin(item)} class="fa-solid fa-thumbtack"></i>
    </Line>)}


    <Typography sx={{mt: 2}} variant="subtitle2">Unpinned searches</Typography>

    {unpinned?.map(item => <Line spacing={2} direction="row" sx={{ alignItems: 'center', width: 400}}>
      <Text onClick={() => handleClose(item)}>{item}</Text>
      <Box sx={{ flexGrow: 1}} />
      <i onClick={() => handlePin(item)} class="fa-solid fa-thumbtack"></i>

      <ConfirmPopover message={`Are you sure you want to delete "${item}"?`} 
          caption="This action cannot be undone!" onChange={(val) => !!val &&  handleDrop(item)} >
     ...
     <pre>
      {JSON.stringify(state.value, 0, 2)}
     </pre>
   </Drawer>
 );
}
