...
import { RegionMenu, Photo, usePhoto, ConfirmPopover } from '..';
...
const VideoCard = ( { video, small, medium, send, favoriteClicked, editClicked, domainClicked, 
    deleteClicked, photoClicked, selected, selectedID, modelClicked, studioClicked }) => {
  ...
  const moreText = models?.length > 1 
    ? <b onClick={() => setExpanded(!expanded)}>+ {models.length - 1} more...</b>
    : <i />
  const costars = models?.length > 1 && expanded ? models.slice(1).map(f => <><ModelName model={f} onClick={() => modelClicked(f.ID)} />, </>) : <i />
  ...
  <Block 
    ...
    > 
      <RegionMenu 
        ...
        {!!editClicked && <i onClick={() => editClicked(video)} className="fa-solid fa-pen"></i>}
        <i onClick={() => favoriteClicked && favoriteClicked(video.ID)} className={`${video.favorite ? "red fa-solid" : "fa-regular"} fa-heart`}></i>
        <ConfirmPopover message={`Are you sure you want to delete "${video.title}"?`} 
          caption="This action cannot be undone!" onChange={(val) => !!val && deleteClicked && deleteClicked(video.ID)} >
        <i className="fa-solid fa-trash-can"></i>
        </ConfirmPopover>
        <i onClick={() => window.open(video.URL)} className="fa-solid fa-up-right-from-square"></i>
        {!!video.Key && <i onClick={() => window.open(`https://www.javlibrary.com/en/vl_searchbyid.php?keyword=${video.Key}`)} className="fa-solid fa-book"></i>}
        </BlockMenu>
      <Tooltip title={video.title}><CardMedia 
        
        onClick={() => setShowRegion(!showRegion)}
        onError={() => source.handleError()}
        component="img"
        sx={{ borderRadius: t => t.spacing(.5,.5,0,0), width: size, aspectRatio: "16 / 9"}}
        width="100%"
        height="auto" 
        image={source.image}
        alt={video.title}
      ...
      <ModelMenu selectedID={selectedID} onChange={e => !!e && modelClicked(e)} 
      models={models}><i className="fa-solid fa-ellipsis-vertical"></i></ModelMenu>
        <ScrollingText
          variant="caption"
          sx={{ whiteSpace: 'nowrap', 
          fontWeight:  !!video.favorite ? 600 : 400,
          textOverflow: 'ellipsis', overflow: 'hidden', width: size - 30 }}
      ...
      {!small && <CardContent sx={{ p: t => t.spacing(1) + ' !important' }}>
        <Stack direction="row" spacing={1}>
          ...
          <Stack> 
              ...
              >
                {costars}
                <ModelName onClick={() => modelClicked(modelProps.ID)} model={modelProps} />{moreText} 
                {/* <u onClick={() => modelClicked(modelProps.ID)}>{modelProps.Name}</u>{" "}*/}
              </Typography>
          </Stack>
        </Stack>
        <Stack direction="row" sx={{justifyContent: 'space-between'}}>
          <Typography onClick={() => domainClicked && domainClicked(video.domain)} variant="caption" >{video.domain}</Typography>
          <Typography onClick={() => studioClicked && studioClicked(video.studio)} variant="caption" >{video.studio}</Typography>
        </Stack>
      </CardContent>}
    </Block>
 );
...
export const ModelMenu = ({ onChange, children, clipText, selectedID, value, models }) => {
        ...
        {models.map((model) => (
          ...
          >
           ...
           <Avatar src={model.image} />
            <Typography
            sx={{ fontWeight: selectedID === model.ID ? 600 : 400 }}>{model.Name}</Typography>
           </Stack>
          </MenuItem>
        ))}
        {!!clipboard && !!clipText && <MenuItem onClick={handleClose(clipboard)}>
          <Stack direction="row" sx={{alignItems: 'center'}} spacing={1}>
